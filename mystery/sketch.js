// https://openprocessing.org/sketch/933378
...
function draw() {
  ...
  if (start == false && t < 4.5) {
    ...
    fill(255);
    textSize(28);
    text("Press mouse to generate", width / 2, height / 2 - 80);
    text("a new random table!", width / 2, height / 2 - 40);
    text("Press S to save!", width / 2, height / 2 + 20);
    ...
    for (let i = 0; i < grid.cells.length; i++) {
      ...
      stroke(
        colors.sample(map(simplex.noise2D(frameCount * cs, i), -1, 1, 0, 1) % 1)
      );
      curvs[i].show();
    }
