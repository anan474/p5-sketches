...
const COLS = createCols(
  "https://coolors.co/242220-ff6a45-ffb094-7934ad-1bc4b6-6a7a5b"
);
...
class Scribbler {
  ...
  display() {
    let rotateAngle = lerp(
      this.rotateAngle,
      this.rotateAngleTarget,
      this.ratio
    );
  ...
  randomVelocities() {
    ...
    this.angleV =
      random() > 0.5
        ? random(-angleSP, -angleSP * 0.1)
        : random(angleSP, angleSP * 0.1);
    if (this.radius < this.minRadius)
      this.radiusV = random(radiusSP * 0.1, radiusSP);
    else if (this.radius > this.maxRadius)
      this.radiusV = random(-radiusSP, -radiusSP * 0.1);
    else
      this.radiusV =
        random() > 0.5
          ? random(radiusSP * 0.1, radiusSP)
          : random(-radiusSP, -radiusSP * 0.1);
  }
}
