// https://openprocessing.org/sketch/1224166
...
function drawSepRect(xMin, yMin, xMax, yMax) {
  ...
  while (y < h) {
    ...
    while (x < w / 2) {
      ...
      drawingContext.shadowBlur = max(xStep, yStep) / 30;
      drawingContext.shadowColor = color(random(palette));
      ellipseMode(CENTER);
      rectMode(CENTER);

      let m = random(0.5, 10);
      if (min(xStep, yStep) > w / 100) {
        let isFill = random() > 0.5;
        let angle = random(90, 270);
        let counter_angle =
          angle > 180
            ? angle + abs(270 - angle) * 2
            : angle - abs(angle - 90) * 2;
        ...
        for (let i = 1.3; i > 0; i -= 1 / sep) {
          ...
          if (isFill) {
            let c = color(color2[(t + v) % colors.length]);
            c.setAlpha(100);
            stroke(c);
            noFill();
          } else {
            let c = color(color2[(t + v) % colors.length]);
            c.setAlpha(100 / sep / 1.5);
            fill(c);
            // noStroke();
            stroke(0, 0, 100, 25);
            strokeWeight(sw / 2);
